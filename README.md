<div align="center">

<img src="https://capsule-render.vercel.app/api?type=waving&color=0:0A66C2,100:0e75b6&height=220&section=header&text=Muhammad%20Umar%20Hayat&fontSize=42&fontColor=ffffff&animation=fadeIn&fontAlignY=38&desc=Software%20Quality%20Assurance%20(SQA)%20Engineer&descAlignY=58&descSize=20" width="100%"/>

<img src="https://readme-typing-svg.demolab.com?font=Fira+Code&weight=600&size=22&pause=1000&color=0A66C2&center=true&vCenter=true&width=650&lines=Manual+QA+%7C+Web%2C+Mobile+%26+SaaS+Testing;API+Testing+%7C+Functional+%26+Regression+Testing;3%2B+Years+of+Breaking+Software+(So+Users+Don't+Have+To)" alt="Typing SVG" />

<br>

<img src="https://komarev.com/ghpvc/?username=umarhayatsqae&label=Profile%20Views&color=0A66C2&style=for-the-badge" alt="Profile Views" />
<img src="https://img.shields.io/badge/Available%20for-Freelance%20%26%20Full--Time-success?style=for-the-badge" alt="Availability" />

<br><br>

<a href="https://www.upwork.com/freelancers/~01f82fbe43248718c1?mp_source=share">
  <img src="https://img.shields.io/badge/Upwork-14A800?style=for-the-badge&logo=upwork&logoColor=white" />
</a>
<a href="mailto:umerhayar6023@gmail.com">
  <img src="https://img.shields.io/badge/Email-D14836?style=for-the-badge&logo=gmail&logoColor=white" />
</a>

</div>

<br>

## 👨‍💻 About Me

<img align="right" width="300" src="https://raw.githubusercontent.com/devSouvik/devSouvik/master/gif3.gif" alt="QA Illustration"/>

I'm a **Software Quality Assurance (SQA) Engineer** with **3+ years of experience** in Manual Testing, specializing in **Web, Mobile, SaaS, and API-based applications**.

🔍 I focus on identifying defects, validating requirements, designing test cases, and performing functional and regression testing to ensure applications are reliable, user-friendly, and release-ready.

🎯 **My mission:** not just finding bugs — but reducing release risk, improving product quality, and delivering a better end-user experience.

- 🧪 3+ years hands-on Manual QA experience
- 🌐 Web, Mobile, SaaS & API testing specialist
- 🐞 Detail-oriented bug hunter with clean, reproducible bug reports
- 🤝 Comfortable working directly with Devs, PMs & Clients (Agile/Scrum)
- 📈 Currently exploring automation fundamentals to complement manual testing

<br clear="right"/>

---

## 🧪 Core QA Expertise

<div align="center">

| Testing Type | Testing Type | Testing Type |
|:---:|:---:|:---:|
| ✅ Manual Testing | 🔁 Regression Testing | ⚡ Smoke Testing |
| 🧭 Sanity Testing | 🔎 Exploratory Testing | 🎨 UI/UX Testing |
| 👤 UAT | 🔗 E2E Testing | 🔌 API Testing |
| 📱 Compatibility Testing | 🖱️ Usability Testing | 🐛 Bug Reporting |
| 📝 Test Case Design | 🌍 Cross-Browser Testing | 📶 Cross-Device Testing |

</div>

---

## 🎯 Testing Domains

<table>
<tr>
<td width="33%" valign="top">

### 🌐 Web Application
- Functional Testing
- Regression Testing
- Smoke & Sanity Testing
- Exploratory Testing
- UI/UX Testing
- Cross-Browser Testing
- Responsive Testing
- End-to-End Testing

</td>
<td width="33%" valign="top">

### 📱 Mobile Application
- Android & iOS Testing
- Functional Testing
- Install & Update Testing
- Device Compatibility
- Push Notification Testing
- Online/Offline Testing
- Usability Testing

</td>
<td width="33%" valign="top">

### 🔌 API Testing
- REST API (GET/POST/PUT/DELETE)
- Request/Response Validation
- Status Code Validation
- Authentication Testing
- Error Handling
- JSON Schema Validation

</td>
</tr>
</table>

---

## 🛠 Tools & Technologies

<div align="center">

**Testing & Bug Tracking**

<img src="https://skillicons.dev/icons?i=postman,jira,figma" /><br>
<img src="https://img.shields.io/badge/TestRail-25A162?style=for-the-badge" />
<img src="https://img.shields.io/badge/Xray-0052CC?style=for-the-badge" />
<img src="https://img.shields.io/badge/Zephyr-0052CC?style=for-the-badge" />
<img src="https://img.shields.io/badge/Bugzilla-CC0000?style=for-the-badge" />
<img src="https://img.shields.io/badge/Mantis-1E4D6B?style=for-the-badge" />

**API & Performance Testing**

<img src="https://img.shields.io/badge/Swagger-85EA2D?style=for-the-badge&logo=swagger&logoColor=black" />
<img src="https://img.shields.io/badge/JMeter-D22128?style=for-the-badge" />
<img src="https://img.shields.io/badge/Postman-FF6C37?style=for-the-badge&logo=postman&logoColor=white" />

**Project & Collaboration**

<img src="https://img.shields.io/badge/Azure%20DevOps-0078D4?style=for-the-badge&logo=azuredevops&logoColor=white" />
<img src="https://img.shields.io/badge/Trello-0052CC?style=for-the-badge&logo=trello&logoColor=white" />
<img src="https://img.shields.io/badge/Asana-F06A6A?style=for-the-badge&logo=asana&logoColor=white" />
<img src="https://img.shields.io/badge/ClickUp-7B68EE?style=for-the-badge&logo=clickup&logoColor=white" />
<img src="https://img.shields.io/badge/Slack-4A154B?style=for-the-badge&logo=slack&logoColor=white" />
<img src="https://img.shields.io/badge/Microsoft%20Teams-6264A7?style=for-the-badge&logo=microsoftteams&logoColor=white" />

**Documentation & Productivity**

<img src="https://skillicons.dev/icons?i=notion" /><br>
<img src="https://img.shields.io/badge/Microsoft%20Excel-217346?style=for-the-badge&logo=microsoftexcel&logoColor=white" />
<img src="https://img.shields.io/badge/Google%20Sheets-34A853?style=for-the-badge&logo=googlesheets&logoColor=white" />

</div>

---

## 🔄 My QA Testing Workflow

<div align="center">

```
📋 Requirement Analysis
        ↓
🧩 Test Scenario Creation
        ↓
📝 Test Case Design
        ↓
▶️  Test Execution
        ↓
🐛 Bug Reporting
        ↓
✅ Bug Verification
        ↓
🔁 Regression Testing
        ↓
🚀 Release Validation
```

</div>

---

## 📋 Test Documentation I Deliver

<table>
<tr>
<td width="33%" align="center">

### 📝 Test Cases
Detailed, requirement-based test cases covering real user workflows and edge cases.

</td>
<td width="33%" align="center">

### 🐛 Bug Reports
Clear reproduction steps, expected vs. actual results, severity, priority & screenshots.

</td>
<td width="33%" align="center">

### 🗺️ Test Scenarios
High-level scenarios mapped to functional requirements and real-world usage.

</td>
</tr>
</table>

---

## 🏢 Industry Experience

<div align="center">

`SaaS Platforms` `E-Commerce` `FinTech` `Healthcare Systems` `HR & Payroll` `Real Estate` `Inventory Management` `Job Portals` `Travel & Booking` `Business Management Systems`

</div>

---

## 📂 QA Portfolio

<div align="center">

<a href="#"><img src="https://img.shields.io/badge/📝_Test%20Cases-View%20Projects-0A66C2?style=for-the-badge" /></a>
<a href="#"><img src="https://img.shields.io/badge/🐛_Bug%20Reports-View%20Projects-CC0000?style=for-the-badge" /></a>
<a href="#"><img src="https://img.shields.io/badge/🔌_API%20Testing-View%20Projects-85EA2D?style=for-the-badge" /></a>
<a href="#"><img src="https://img.shields.io/badge/📱_Mobile%20Testing-View%20Projects-8A2BE2?style=for-the-badge" /></a>

</div>

---

## 📊 GitHub Stats

<div align="center">

<img src="https://github-readme-stats.vercel.app/api?username=umarhayatsqae&show_icons=true&locale=en&theme=tokyonight&hide_border=true&count_private=true" alt="Umar Hayat GitHub Stats" height="165"/>
<img src="https://github-readme-streak-stats.herokuapp.com/?user=umarhayatsqae&theme=tokyonight&hide_border=true" alt="GitHub Streak" height="165"/>

<br>

<img src="https://github-readme-stats.vercel.app/api/top-langs/?username=umarhayatsqae&layout=compact&theme=tokyonight&hide_border=true" alt="Top Languages" />

</div>

---

## 🎯 What I Can Help With

<div align="center">

✔ Manual QA &nbsp;•&nbsp; ✔ Web Testing &nbsp;•&nbsp; ✔ Mobile Testing &nbsp;•&nbsp; ✔ SaaS Testing &nbsp;•&nbsp; ✔ API Testing
✔ Functional & Regression Testing &nbsp;•&nbsp; ✔ Smoke & Sanity Testing &nbsp;•&nbsp; ✔ Exploratory Testing
✔ Cross-Browser & Cross-Device Testing &nbsp;•&nbsp; ✔ Test Case Creation &nbsp;•&nbsp; ✔ Bug Reporting
✔ UAT &nbsp;•&nbsp; ✔ Release Testing &nbsp;•&nbsp; ✔ Ongoing QA Support

</div>

---

## 📚 Currently Improving

<div align="center">

🚀 Continuously sharpening my QA skills and exploring modern testing tools, automation fundamentals, and industry best practices to deliver better quality coverage.

</div>

---

## 🤝 Let's Connect

<div align="center">

<a href="https://www.upwork.com/freelancers/~01f82fbe43248718c1?mp_source=share">
  <img src="https://img.shields.io/badge/Upwork-14A800?style=for-the-badge&logo=upwork&logoColor=white" />
</a>
<a href="mailto:umerhayar6023@gmail.com">
  <img src="https://img.shields.io/badge/Gmail-D14836?style=for-the-badge&logo=gmail&logoColor=white" />
</a>

📧 **umerhayar6023@gmail.com**

💼 *Open to SQA / Manual QA opportunities, freelance projects, and QA collaborations.*

</div>

<br>

<div align="center">

### ⭐ Thanks for visiting my profile!

> *"Quality is not just about finding bugs — it's about delivering confidence."*

<img src="https://capsule-render.vercel.app/api?type=waving&color=0:0e75b6,100:0A66C2&height=100&section=footer" width="100%"/>

</div>
